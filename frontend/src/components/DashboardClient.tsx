...
import { listFiles, getSiaStatus, getFile } from '@/lib/api'
...
export default function DashboardClient() {
  ...
  const [siaStatus, setSiaStatus] = useState<SiaStatus | null>(null)
  const [setupDone, setSetupDone] = useState(false)

  // ── Fetch Sia status on mount ─────────────────────────────────────────────
  useEffect(() => {
    getSiaStatus().then((status) => {
      setSiaStatus(status)
      if (!status.approvalPending) setSetupDone(true)
    })
  }, [])

  // ── Load existing files once setup is confirmed ───────────────────────────
  useEffect(() => {
    if (!setupDone) return
    listFiles().then((remote) => {
      if (remote.length) setFiles(remote)
    })
  }, [setupDone])
  ...
  const upsertFile = useCallback((record: FileRecord) => {
    setFiles((prev) => {
      ...
      // Not found — prepend (covers the optimistic placeholder case)
      return [record, ...prev]
    })
  }, [])
  ...
  /**
   * Called by UploadModal once the backend has accepted the file and returned
   * a fileId. We immediately fetch that file record so the card appears in the
   * list right away (status: 'queued' or 'uploading'), then start the poller.
   */
  const handleUploaded = useCallback(async (fileId: string) => {
    setExpandedId(fileId)
    addToast('info', 'Pipeline started — tracking progress…')

    // Optimistic fetch: insert the card as soon as the backend has the record.
    // This runs once immediately so the card is visible before the poller fires.
    try {
      const record = await getFile(fileId)
      upsertFile(record)
    } catch {
      // If the first fetch fails, the poller will pick it up shortly anyway.
    }
  ...
  // Still loading status — show minimal spinner
  if (siaStatus === null) {
    return (
  ...
  return (
    <>
      {!setupDone && siaStatus.approvalPending && siaStatus.approvalUrl && (
        <SetupModal
          approvalUrl={siaStatus.approvalUrl}
          onConnected={() => {
            setSetupDone(true)
            addToast('success', "Sia connected — you're all set!")
          }}
        />
      )}
